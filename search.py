...
class BoardSearch(SearchProblem):
    """
    Starting point is a target
    States are points
    Actions are moving up, down, left or right on the board
    ...
    def is_goal_state(self, state):
        ...
        Returns True if and only if the state is a valid goal state
        """
        return self.board.connected[0][state[STATE_Y]][state[STATE_X]]
        # util.raiseNotDefined()
    ...
    def get_successors(self, state):
        ...
        successors = []
        if state[STATE_X] > 0:
            successors.append(((state[STATE_X] - 1, state[STATE_Y]), None, 1))

        if state[STATE_X] < self.board.board_w - 1:
            successors.append(((state[STATE_X] + 1, state[STATE_Y]), None, 1))

        if state[STATE_Y] > 0:
            successors.append(((state[STATE_X], state[STATE_Y] - 1), None, 1))

        if state[STATE_Y] < self.board.board_w - 1:
            successors.append(((state[STATE_X], state[STATE_Y] + 1), None, 1))
        return successors
        # util.raiseNotDefined()
